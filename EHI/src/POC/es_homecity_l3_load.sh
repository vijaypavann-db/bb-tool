...
,DTE_END_CST_DATE
 ...
 WHERE
   ...
   AND CLS.CREATE_TIMESTAMP >= CAST((dte.START_CST_DATE - 7 ) AS TIMESTAMP FORMAT 'YYYY-MM-DD:HH:MI:SS')                          /* performance */
...
CREATE  VOLATILE TABLE   VT_RTM_INSERTS AS 
(
select 
...
CREATE VOLATILE TABLE TKT_TEMP1 AS
(
SELECT
...
/* changes end January 2015	*/
     ...
     , cst.account_nbr AS RATE_SRCE_CUST_NBR    /* Modified fot CUST_MAST decommissioning DW CQ RENTL00232659 */
     ...
     , tkt.BTI_NCLD_FLG  /* Added for  DW CQ RENTL00232659 */ 
     , tkt.gui_resv_nbr
...
/* Modified fot CUST_MAST decommissioning DW CQ RENTL00232659 */ 
 ...
 WHERE
    SYSLIB.ERAC_GMT_TO_LCL(cls.CREATE_TIMESTAMP,ORG.TIM_ZON_CDE) >=
        CAST((cls.dte_START_CST_DATE - 6 ) AS TIMESTAMP FORMAT 'YYYY-MM-DD:HH:MI:SS')
...
CREATE VOLATILE TABLE TKT_TEMP2 AS
 ( 
SELECT
       TKT.RTN_MTH_KEY_ID
     , TKT.OPN_IORG_ID
...
/*	changes end January 2015	*/
     ...
     , TKT.GUI_RESV_NBR
     , TKT.ORIG_RWRT_RENT_CNTRCT_NBR
     , SUM(CASE WHEN TRIM(DTL.RENT_CNTRCT_CHRG_TYP_CDE)  = '00001' THEN DTL.CLOS_TKT_TOT_DTL_CHRG_AMT
               ELSE 0.00
          END) AS TD_AMT
          
     , MIN(CASE                                                /*Eligible DW Days*/
            WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 AND TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00006' 
              THEN 0
   	        ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
            END) AS  ELGBL_DW_DAYS_QTY  
...
/* remove excess from calculations*/
     , MIN(CASE                           /*Eligible PAI  00209 = PAI/PEC AND 00007 = PAI */
       WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 
       		...
       		THEN 0
       ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
        END) AS  elgbl_pai_days_qty         
        
/* remove excess from calculations*/
     , SUM(CASE                         /*PAI  00209 = PAI/PEC AND 00007 = PAI  */
...
/* changes end  May 2014	*/
     ...
     , MIN(CASE                                                           /*Eligible SLP Days*/
       WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 AND TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00032' THEN 0
       WHEN tkt.BTI_NCLD_FLG = 1  THEN 0  /* bti added June, 2015 fo for  DW CQ RENTL00232659  */ 
       ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
        END) AS  elgbl_slp_days_qty  
     ...
     , MIN(CASE                           /*Eligible GPS  00247 = GPS UNIT- CAR   00254 = GPS UNIT  00224= SAT NAV Days*/
      WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 
	    AND
	    ...
	    OR TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00224') 		
       	THEN 0
      ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
       END) AS  elgbl_gps_days_qty  
        
     , SUM(CASE                 /*GPS  00247 = GPS UNIT- CAR   00254 = GPS UNIT  00224= SAT NAV*/
...
/* changes start May 2014*/
/* modifications start*/
     , MIN(CASE                           /*Eligible RAP  00279 = RAP AND 05033 = TKR */
       WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 
       		AND 
       		...
       		THEN 0
       ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
        END) AS  elgbl_rap_days_qty           



     , SUM(CASE                         /*RAP  00209 = RAP AND 05033 = TKR  */
...
/* modifications end*/
   ...
   /* changes end  May 2014	*/    
     ...
     , MIN(CASE      /*Eligible Pre-Paid Fuel Days*/
       WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 
       AND 
       (TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00035' 
        OR UPPER(TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE)) = 'F')
       	THEN 0
        ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
        END) AS  elgbl_pre_paid_fuel_days_qty          
                
        
     , SUM(CASE                                                /* Pre-Paid Fuel */
     ...
     , MIN(CASE                           /*Eligible 00282 - SATELLITE RADIO  - CAR, 00283 - SATELLITE RADIO - TRUCK  */
       WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1 
       		AND 
        		(
       		...
       		THEN 0
       ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
        END) AS  elgbl_satellite_radio_days_qty         

     , SUM(CASE                          /* 00282 - SATELLITE RADIO  - CAR, 00283 - SATELLITE RADIO - TRUCK	*/
       WHEN TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00282' 
            OR TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '00283' 
            THEN dtl.CLOS_TKT_TOT_DTL_CHRG_AMT
       ELSE 0.00 
       END) AS SATELLITE_RADIO_AMT

, MIN(CASE                                                       /*Eligible COE Days*/
                WHEN dtl.CLOS_TKT_TOT_DTL_NCLV_IND = 1  AND (TRIM(dtl.RENT_CNTRCT_CHRG_TYP_CDE) = '05234' )
       	  THEN 0
                ELSE TKT.RENT_CNTRCT_TOT_DAY_CHRG_QTY
                END) AS  elgbl_coe_days_qty          
		...
		CASE            
							...
							THEN dtl.CLOS_TKT_TOT_DTL_CHRG_AMT                
		                    ELSE 0.00                 
		END)AS ADDL_TRUCK_PRODUCTS_AMT
...
CREATE VOLATILE TABLE Ody_res_temp1 AS
(
...
,	MAX (	CASE WHEN dtl.rent_cntrct_chrg_typ_cde = 'ARPT'  AND dtl.perd_Typ_cde =6 THEN dtl.clos_tkt_tot_dtl_chrg_rate_amt ELSE 0.00 END)/100 AS PLF_percent
/* Premium Location Fee Charged based per Rental Fee*/
,	MAX (	CASE WHEN dtl.rent_cntrct_chrg_typ_cde = 'ARPT'  AND dtl.perd_Typ_cde =4 THEN dtl.clos_tkt_tot_dtl_chrg_rate_amt ELSE 0.00 END) AS PLF_per_rental
/* Premium Location Fee Charged based on a per Day*/
,	MAX (	CASE WHEN dtl.rent_cntrct_chrg_typ_cde = 'ARPT'  AND dtl.perd_Typ_cde IN(1,2,3,7) THEN dtl.clos_tkt_tot_dtl_chrg_rate_amt ELSE 0.00 END) AS PLF_per_day
/* Value Added Tax based on a Percentage*/
,	MAX( 	CASE WHEN dtl.rent_cntrct_chrg_typ_cde IN ('SVAT','VAT','IVA') THEN dtl.clos_tkt_tot_dtl_chrg_rate_amt ELSE 0.00 END)/100 AS VAT_percent
				...
				AND 	rcc.rent_cntrct_chrg_ncld_ind =1  /* tax included*/		
...
CREATE VOLATILE TABLE Ody_res_rate AS
(
...
END 	AS DECIMAL (20,10) )  AS calc_Res_Rate			
		FROM 
		TKT_TEMP1 tkt 
		INNER JOIN 
		...
		AND res.SRCE_CHNL_CDE = 1  /* Odyssey Reservation*/
		JOIN 
...
CREATE VOLATILE TABLE Ody_Tour_Res_Charges AS
(
...
CREATE VOLATILE TABLE SELLUP_TMP AS
 ( 
	Select
	OPN_IORG_ID
	,RENT_CNTRCT_NBR
...
Add new start May 2014
*/
		  --Odyssey created tax inclusive reservation
...
*/
		...
		-- When reservation's daily rate amount is 0 and arms_auth_id = 1
		--   get "legacy quote rate amount" from rent_cntrct as reservation rate
   ,CASE
   ...
   ,CASE
     ...
     WHEN res.arms_auth_ind = 1 AND res.lgcy_quote_rate_amt > 0.000 AND resRt.VEH_DAY_RATE_AMT = 0.000 
	     ...
	     THEN  tktDtl.CLOS_TKT_TOT_DTL_CHRG_AMT - lgcyResChrg
			...
			--This was added to check car class reserved vs. car class charged for ARMS reservations                          
			-- in Europe.  We have to check the car class on the Res as well as the Car Class Reserved on the ticket
			-- because we found an issue where they can get out of sync.  
			-- car class reserved as it is on the ticket

			WHEN '${CHCK_CAR_CLSS}' = 'Y' AND tkt.CAR_CLS_RSVD = tkt.CAR_CLS_CHRGD 
			AND   tkt.RESV_ORIG_CDE = 1 THEN 0.000

			-- car class on the reservation
			WHEN '${CHCK_CAR_CLSS}' = 'Y' AND resRt.CAR_CLS_CDE= tkt.CAR_CLS_CHRGD 
			AND tkt.RESV_ORIG_CDE = 1 THEN 0.000

			--No sellup if non-ARMS res, less than 1 day and less than 1 hour between res create date/time and ticket pickup date/time
			-- NOTE : As of Feb 2016 - Release 16.04 - this 1 hour sellup for non-ARMS EU data was asked to be removed
			--        The country parameter CHCK_RES_TO_TKT_TM is forced to set to 'N' in this script
			--        but the business rule is kept in the code for any future use.
...
/*	new changes end January 2015			*/
     ...
     , TKT.COE_AMT
     , CAST(CURRENT_TIMESTAMP as TIMESTAMP(6)) as DW_TSP
  FROM TKT_TEMP2 TKT
...
/*	new changes end January 2015			*/                
 ...
 WHERE     
       UPPER(TRIM(ORG.CNTRY_ISO_CDE)) NOT IN (${CNTRY_CDE})
        ...
        OR
        (  FCT.TKT_CLOS_TSP < 
           CAST((DTE.START_CST_DATE - 6 )AS TIMESTAMP FORMAT 'YYYY-MM-DD:HH:MI:SS')
        OR 
