{% set lookback_predicate %}
    CAST(event_date_time_pt AS DATE) =  TO_DATE({{ get_run_date() }})
{% endset %}
...
SELECT
  ...
  candidates
FROM
    {{ ref('vw_replacement_customer') }}
LATERAL VIEW OUTER EXPLODE(item_card_impressions) AS i
WHERE
    (
        event IN ('store.replacements_view',
    ...
    OR (
        event IN ('store.item_card_impressions')
...
-- Addl. filter added for partition filtering
AND dt >={{ get_run_date_w_interval('14 days') }}
{% endif %}
